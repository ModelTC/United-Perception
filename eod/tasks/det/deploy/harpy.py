...
class HarpyParser(eod_parser.Parser):
...
def split_net(net, anchor_num, cls_channel_num, bbox_head_type):
    ...
    # process reshape
    eod_parser.process_reshape(rpn_net, anchor_num, 2, anchor_precede=False)
    eod_parser.process_reshape(det_net, 1, cls_channel_num, anchor_precede=False)
...
def generate_config(train_cfg):
    ...
    kestrel_net_param['det_nms_thresh'] = model.post_process.predictor.nms_cfg['nms_iou_thresh']
    ...
    kestrel_param['class'] = eod_parser.get_forground_class_threshes(train_cfg['dataset'], train_cfg['to_kestrel'])
