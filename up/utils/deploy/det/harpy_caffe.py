...
@KS_PARSER_REGISTRY.register('harpy_caffe')
class HarpyParser_caffe(up_parser.Parser):
...
@KS_PROCESSOR_REGISTRY.register('harpy_caffe')
class HarpyProcessor_caffe(BaseProcessor):
    def process(self):
        ...
        scaffold.compress_model(
            self.save_path, [net_info['rpn']['packname'], net_info['det']['packname']], self.name, version)
