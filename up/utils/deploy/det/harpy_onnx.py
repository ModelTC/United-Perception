...
from up.utils.deploy.det.harpy import generate_common_param, generate_config


@KS_PARSER_REGISTRY.register('harpy_onnx')
class HarpyParser_onnx(up_parser.Parser):
...
@KS_PROCESSOR_REGISTRY.register('harpy_onnx')
class HarpyProcessor_onnx(BaseProcessor):
    def process(self):
        ...
        scaffold.compress_model(
            self.save_path, ['rpn.onnx', 'det.onnx'], self.save_path, version)
