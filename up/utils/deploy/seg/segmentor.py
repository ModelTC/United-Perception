...
try:
    ...
    print('No module named spring in up/utils/deploy/seg/segmentor.py')
...
__all__ = ['SegmentorParser']


@KS_PARSER_REGISTRY.register('segmentor')
class SegmentorParser(up_parser.Parser):
    def get_kestrel_parameters(self):
        return generate_config(self.cfg)


def parse_dataset_param(dataset_cfg, default_thresh):
    ...
    color_mode = up_parser.getdotattr(kwargs_cfg,
                                      'image_reader.kwargs.color_mode')
    ...
    num_classes = kwargs_cfg['num_classes']
    if 'class_names' in kwargs_cfg:
        class_names = kwargs_cfg['class_names']
    else:
        class_names = [f'cls{i}' for i in range(num_classes)]
        class_names[0] = 'background'

    dataset_param['num_semantic_classes'] = num_classes
    dataset_param['calculate_instance'] = False
    dataset_param['class'] = [{
        "id":
        idx,
        "label":
        cls,
        "thresh":
        0 if 'background' in cls else default_thresh,
...
def generate_config(train_cfg):
    ...
    to_kestrel_param = train_cfg['to_kestrel']
    thresh = to_kestrel_param.get('default_thresh', 0.3)

    dataset_param = parse_dataset_param(train_cfg['dataset'], thresh)
...
def generate_common_param(net_info, max_batch_size):
    ...
    for i, pair in enumerate(net_info['output']):
        net_param['output']['sem_out'] = pair
    common_param['net'] = net_param
    return common_param


@KS_PROCESSOR_REGISTRY.register('segmentor')
class SegmentorProcessor(BaseProcessor):
    def process(self):
        ...
        net, net_info, out_info = process_net(self.prototxt, self.model,
                                              kestrel_param['input_h'],
                                              kestrel_param['input_w'],
                                              self.input_channel)
        ...
        scaffold.generate_meta(self.save_path, self.name, 'segmentor', version)
        pack_list = [net_info['packname']]
        scaffold.compress_model(self.save_path, pack_list, self.save_path,
                                version)
