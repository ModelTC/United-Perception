...
def image_box_overlap(boxes, query_boxes, criterion=-1):
    ...
    for index_k in range(K):
        qbox_area = ((query_boxes[index_k, 2] - query_boxes[index_k, 0])
                     * (query_boxes[index_k, 3] - query_boxes[index_k, 1]))
        for n in range(N):
            iw = (min(boxes[n, 2], query_boxes[index_k, 2])
                  - max(boxes[n, 0], query_boxes[index_k, 0]))
            if iw > 0:
                ih = (min(boxes[n, 3], query_boxes[index_k, 3])
                      - max(boxes[n, 1], query_boxes[index_k, 1]))
                if ih > 0:
                    if criterion == -1:
                        ua = (
                            (boxes[n, 2] - boxes[n, 0])
                        ...
                        ua = 1.0
                    overlaps[n, index_k] = iw * ih / ua
    return overlaps


def bev_box_overlap(boxes, qboxes, criterion=-1):
    from up.tasks.det_3d.data.metrics.kitti_object_eval_python.rotate_iou import rotate_iou_gpu_eval
...
def eval_class(gt_annos,
    ...
    for index_m, current_class in enumerate(current_classes):
        for index_l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty)
            (gt_datas_list, dt_datas_list, ignored_gts, ignored_dets,
             dontcares, total_dc_num, total_num_valid_gt) = rets
            for index_k, min_overlap in enumerate(min_overlaps[:, metric, index_m]):
                thresholdss = []
                for i in range(len(gt_annos)):
                ...
                for i in range(len(thresholds)):
                    recall[index_m, index_l, index_k, i] = pr[i, 0] / (pr[i, 0] + pr[i, 2])
                    precision[index_m, index_l, index_k, i] = pr[i, 0] / (pr[i, 0] + pr[i, 1])
                    if compute_aos:
                        aos[index_m, index_l, index_k, i] = pr[i, 3] / (pr[i, 0] + pr[i, 1])
                for i in range(len(thresholds)):
                    precision[index_m, index_l, index_k, i] = np.max(
                        precision[index_m, index_l, index_k, i:], axis=-1)
                    recall[index_m, index_l, index_k, i] = np.max(recall[index_m, index_l, index_k, i:], axis=-1)
                    if compute_aos:
                        aos[index_m, index_l, index_k, i] = np.max(aos[index_m, index_l, index_k, i:], axis=-1)
    ret_dict = {
        "recall": recall,
        "precision": precision,
        "orientation": aos,
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    compute_aos = False
    for anno in dt_annos:  # list of dict
    ...
    for j, curcls in enumerate(current_classes):
        ...
        result_dict[class_to_name[curcls]] = {}
        for i in range(min_overlaps.shape[0]):

            result_dict_ap = {}
            result_dict_ap["bbox AP"] = mAPbbox[j, :, i].tolist()
            result_dict_ap["bev  AP"] = mAPbev[j, :, i].tolist()
            result_dict_ap["3d   AP"] = mAP3d[j, :, i].tolist()
            if compute_aos:
                result_dict_ap["aos  AP"] = mAPaos[j, :, i].tolist()

            result_dict_ap_r40 = {}
            result_dict_ap_r40["bbox AP"] = mAPbbox_R40[j, :, i].tolist()
            result_dict_ap_r40["bev  AP"] = mAPbev_R40[j, :, i].tolist()
            result_dict_ap_r40["3d   AP"] = mAP3d_R40[j, :, i].tolist()
            if compute_aos:
                result_dict_ap_r40["aos  AP"] = mAPaos_R40[j, :, i].tolist()
                if i == 0:
            ...
            result_dict[class_to_name[curcls]].update(
                {"AP@{:.2f}, {:.2f}, {:.2f}:".format(*min_overlaps[i, :, j]): result_dict_ap})
            result_dict[class_to_name[curcls]].update(
                {"AP_R40@{:.2f}, {:.2f}, {:.2f}:".format(*min_overlaps[i, :, j]): result_dict_ap_r40})
